...
START /WAIT RunCmdX.exe ALU_PreloadUtility_Detect.cmd
...
:SUCCESS
SET EXITCODE=0
START /B /WAIT HTTP2GA.exe Fixpack3-006 Fixpack-D NeedUpdate
ECHO %EXITCODE% >> FpCheckLog.txt
GOTO END

:FAIL
SET EXITCODE=1
START /B /WAIT HTTP2GA.exe Fixpack3-010 Fixpack-D Don'tNeedUpdate
START /B /WAIT HTTP2GA.exe Fixpack3-010 Don'tNeedUpdate %SERIAL%
ECHO %EXITCODE% >> FpCheckLog.txt
GOTO END
